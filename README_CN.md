...
MaverickJ 是一个基于多 Agent 协作的对抗式决策分析引擎。用户输入一个决策问题后，系统启动 **4 个具有不同角色定位的 AI Agent**，通过多轮结构化辩论（正方论证 → 反方反驳 → 事实校验 → 主持人裁决），模拟真实的决策审议过程，最终输出一份包含正反论点、关键分歧、风险评估和行动建议的 **结构化决策报告**。

AI 可以用大量的知识去辅助人类的决策，但是不要让 AI 取代了你的思考。
杠精的存在是为了尽可能消除 RLHF（Reinforcement Learning from Human Feedback）对于人类的影响。
...
- **Python** >= 3.12
- **[uv](https://github.com/astral-sh/uv)**（推荐：本地安装与锁文件驱动流程）
...
# 只有在 pyproject.toml 或 uv.lock 变更后才需要重新 build。

# 运行测试（镜像默认只装运行时依赖；首次会拉取 dev 依赖再跑 pytest）：
...
uv run debate-interactive              # 交互式模式
```
...
```bash
uv sync
uv run python -m maverickj.main "我们应该将 Java 后端迁移到 Go 吗？" "50人团队，Spring Boot 3年"
