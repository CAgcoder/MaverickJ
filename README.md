...
用户问题 + 历史记录
      │
      ▼
Step 1: round_setup  ──→ 递增轮次、清空临时字段
      │
      ▼
Step 2: advocate     ──→ 提出/修正正方论点 + 反驳对方
      │
      ▼
Step 3: critic       ──→ 反驳正方论点 + 提出反方论点
      │
      ▼
Step 4: fact_checker ──→ 校验双方论点的逻辑/事实准确性
      │
      ▼
Step 5: moderator    ──→ 总结本轮 + 计算收敛 + 决定是否继续
      │
      ├── should_continue=True  → 回到 round_setup
...
active ──→ modified（被部分反驳后修正）
  │
  ├──→ rebutted（被有效反驳 或 Fact-Checker 判定 flawed → 自动标记）
  │
  └──→ conceded（持有者主动承认让步）
```
